// Step 1 + Step 2 of the 2002 overhaul: audit the CURRENT (before) ratings and
// roster completeness for every 2002 team, grounded in the box-score snapshot
// reports/_work/2002_before.csv and the corrected registry (records/strength/
// names from data/team_seasons.csv). Read-only; writes four reports:
//
//   reports/2002_suspicious_teams.csv     distribution red-flags per team
//   reports/2002_suspicious_players.csv   players whose rating contradicts their box score
//   reports/2002_roster_completeness.csv  position-group head-count per team
//   reports/2002_rating_audit.md          narrative summary
//
// Run: node scripts/audit-2002.mjs
...
const rows = readCsv(path.join(WORK, "2002_before.csv"));
const registry = readCsv(path.join(ROOT, "data", "team_seasons.csv")).filter((r) => r.season === "2002");

const regByCode = new Map(registry.map((r) => [r.team_code, r]));
const N = (v) => (v === "" || v == null ? null : Number(v));
const ne = (v) => v != null && String(v).trim() !== "";

// Playoff fields & SB-class teams (2002) for the floor checks.
// 2002 playoff teams: AFC = OAK(LV), TEN, PIT, NYJ, IND, CLE; NFC = TB, PHI, GB, SF, NYG, ATL.
const PLAYOFF = new Set(["LV", "TEN", "PIT", "NYJ", "IND", "CLE", "TB", "PHI", "GB", "SF", "NYG", "ATL"]);
// Teams that indisputably fielded a 90+ superstar in 2002 (MVP/DPOY/OPOY/award/All-Pro core):
// LV (Gannon MVP), KC (Priest Holmes OPOY, record TDs), TB (Brooks DPOY), IND (Manning + Harrison
// 143-rec record), MIN (Randy Moss), MIA (Ricky Williams rush title, Jason Taylor), BAL (Ray Lewis),
// LAC (LaDainian Tomlinson), GB (Brett Favre), LAR (Faulk/Holt).
const SB_CLASS = new Set(["LV", "KC", "TB", "IND", "MIN", "MIA", "BAL", "LAC", "GB", "LAR"]);
...
for (const code of codes) {
  ...
  const action = flags.length
    ...
    : "OK — spot-check only.";
  teamReport.push({
    team_code: code, team_name: teamName, season: 2002,
    players_total: total,
  ...
  completeness.push({
    team_code: code, team_name: teamName, season: 2002, total_players: total,
  ...
  for (const r of list) {
    ...
    if (floor && ovr != null && ovr < floor - 2) {
      playerReport.push({
        ...
        season: 2002, position: r.position, current_overall: ovr,
...
writeCsv(path.join(OUT, "2002_suspicious_teams.csv"), teamReport,
...
writeCsv(path.join(OUT, "2002_suspicious_players.csv"), playerReport,
  ["player_id", "name", "team_code", "team_name", "season", "position", "current_overall", "issue", "recommended_action"]);
writeCsv(path.join(OUT, "2002_roster_completeness.csv"), completeness,
...
const md = [];
md.push("# 2002 Rating Audit — Before State\n");
md.push(`Generated from \`reports/_work/2002_before.csv\` (${rows.length} player-rows, ${codes.length} teams) against the corrected 2002 registry.\n`);
md.push("## Summary\n");
md.push(`- **${flagged.length} of ${codes.length} teams** trip at least one distribution red-flag.`);
md.push(`- **${playerReport.length} players** carry a rating that contradicts their own 2002 box score (or are full-time starters stuck in the 67-71 band).`);
md.push(`- The dominant failure mode is **rating compression**: position groups with no recorded starts/stats (all of defense, the OL, and specialists — defensive box scores and awards are empty for 2002) collapse onto a neutral ~68 base, so genuine starters and a few stars are indistinguishable from deep backups.`);
md.push(`- A secondary failure mode is **flat depth**: the engine's per-player depth dispersal is currently scoped to 1999/2000 only, so 2002's statless depth stacks onto a single team-scaled number. The overhaul extends the dispersal set to 2002.`);
md.push(`- **No curated overrides** exist for 2002 today, so every rating is engine-generated.\n`);
