// Extract every 2002 player from the exported team-season JSONs into one flat
...
// for the curated override pass. Read-only; writes reports/_work/2002_before.csv.
...
const files = fs.readdirSync(DIR).filter((f) => /^2002_/.test(f) && f.endsWith(".json")).sort();
const rows = [];
...
writeCsv(path.join(OUTDIR, "2002_before.csv"), rows, cols);
console.log(`extracted ${rows.length} player-rows from ${files.length} teams -> reports/_work/2002_before.csv`);
