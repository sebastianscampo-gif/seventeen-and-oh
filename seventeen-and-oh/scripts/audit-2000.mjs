// Step 1 + Step 2 of the 2000 overhaul: audit the CURRENT (before) ratings and
// roster completeness for every 2000 team, grounded in the box-score snapshot
// reports/_work/2000_before.csv and the corrected registry (records/strength/
// names from data/team_seasons.csv). Read-only; writes four reports:
//
//   reports/2000_suspicious_teams.csv     distribution red-flags per team
//   reports/2000_suspicious_players.csv   players whose rating contradicts their box score
//   reports/2000_roster_completeness.csv  position-group head-count per team
//   reports/2000_rating_audit.md          narrative summary
//
// Run: node scripts/audit-2000.mjs
...
const rows = readCsv(path.join(WORK, "2000_before.csv"));
const registry = readCsv(path.join(ROOT, "data", "team_seasons.csv")).filter((r) => r.season === "2000");

const regByCode = new Map(registry.map((r) => [r.team_code, r]));
const N = (v) => (v === "" || v == null ? null : Number(v));
const ne = (v) => v != null && String(v).trim() !== "";

// Playoff fields & SB-class teams (2000) for the floor checks.
// 2000 playoff teams: AFC = TEN, OAK(LV), BAL, MIA, DEN, IND; NFC = NYG, MIN, NO, PHI, STL(LAR), TB.
const PLAYOFF = new Set(["TEN", "LV", "BAL", "MIA", "DEN", "IND", "NYG", "MIN", "NO", "PHI", "LAR", "TB"]);
// Teams that indisputably fielded a 90+ superstar in 2000 (MVP/DPOY/All-Pro core):
// BAL (Ray Lewis DPOY), LAR (Marshall Faulk MVP), MIN (Randy Moss), IND (James/Harrison/Manning),
// TEN, LV (Gannon/Brown), NYG (Michael Strahan).
const SB_CLASS = new Set(["BAL", "LAR", "MIN", "IND", "TEN", "LV", "NYG"]);
...
for (const code of codes) {
  ...
  const action = flags.length
    ...
    : "OK — spot-check only.";
  teamReport.push({
    team_code: code, team_name: teamName, season: 2000,
    players_total: total,
  ...
  completeness.push({
    team_code: code, team_name: teamName, season: 2000, total_players: total,
  ...
  for (const r of list) {
    ...
    if (floor && ovr != null && ovr < floor - 2) {
      playerReport.push({
        ...
        season: 2000, position: r.position, current_overall: ovr,
...
writeCsv(path.join(OUT, "2000_suspicious_teams.csv"), teamReport,
...
writeCsv(path.join(OUT, "2000_suspicious_players.csv"), playerReport,
  ["player_id", "name", "team_code", "team_name", "season", "position", "current_overall", "issue", "recommended_action"]);
writeCsv(path.join(OUT, "2000_roster_completeness.csv"), completeness,
...
const md = [];
md.push("# 2000 Rating Audit — Before State\n");
md.push(`Generated from \`reports/_work/2000_before.csv\` (${rows.length} player-rows, ${codes.length} teams) against the corrected 2000 registry.\n`);
md.push("## Summary\n");
md.push(`- **${flagged.length} of ${codes.length} teams** trip at least one distribution red-flag.`);
md.push(`- **${playerReport.length} players** carry a rating that contradicts their own 2000 box score (or are full-time starters stuck in the 67-71 band).`);
md.push(`- The dominant failure mode is **rating compression**: position groups with no recorded starts/stats (all of defense, the OL, and specialists — defensive box scores and awards are empty for 2000) collapse onto a neutral ~68 base, so genuine starters and a few stars are indistinguishable from deep backups.`);
md.push(`- A secondary failure mode is **flat depth**: the engine's per-player depth dispersal is currently scoped to 1999 only, so 2000's statless depth stacks onto a single team-scaled number. The overhaul extends the dispersal set to 2000.`);
md.push(`- Only **4 curated overrides** exist for 2000 today (all Baltimore: Ray Lewis, Jonathan Ogden, Rod Woodson, Shannon Sharpe), so every other rating is engine-generated.\n`);
