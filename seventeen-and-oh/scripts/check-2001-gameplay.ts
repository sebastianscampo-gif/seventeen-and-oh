// Offline gameplay smoke test for the 2001 season overhaul.
// Exercises the REAL gameplay code paths against the REAL exported 2001 JSONs:
//   1. Draft pool / catalog: index.json filtered to 2001 (what the spinner draws).
...
//   npx tsc scripts/check-2001-gameplay.ts --outDir /tmp/gp01 --module commonjs \
//     --target es2022 --moduleResolution node --skipLibCheck --esModuleInterop --resolveJsonModule
//   node /tmp/gp01/scripts/check-2001-gameplay.js
...
const pool01 = index.filter((r) => r.season === SEASON);
console.log(`Draft catalog (2001): ${pool01.length} team-seasons`);
...
console.log("\nTeam profiles (real computeTeamProfile on 2001 rosters), by power:");
...
console.log(`\nTotal 2001 players: ${allPlayers}. Most common OVR values:`);
