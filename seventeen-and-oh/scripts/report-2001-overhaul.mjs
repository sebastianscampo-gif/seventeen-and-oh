// report-2001-overhaul.mjs
// Read-only reporting pass for the 2001 rating overhaul. Emits, in one run:
//   Step 5: reports/2001_manual_review_queue.csv
//   Step 7: reports/2001_team_rating_summaries.md
//   Step 8: reports/2001_rating_before_after.csv
//   Step 8: reports/2001_team_distribution_before_after.csv
//
// Sources of truth:
//   BEFORE  -> reports/_work/2001_before.csv  (pre-overhaul engine state)
//   AFTER   -> public/team-seasons/2001_<TC>.json  (post-overhaul exported state)
//
// Players are joined BEFORE<->AFTER by player_id (handles the few 2001 roster
// corrections where a player moved teams). Nothing here mutates game data.
//
// Run:  node scripts/report-2001-overhaul.mjs
...
const SEASON = 2001;
const BEFORE_CSV = fromRoot("reports", "_work", "2001_before.csv");
const REGISTRY_CSV = fromRoot("data", "team_seasons.csv");
const TS_DIR = fromRoot("public", "team-seasons");

const OUT_QUEUE = fromRoot("reports", "2001_manual_review_queue.csv");
const OUT_SUMMARIES = fromRoot("reports", "2001_team_rating_summaries.md");
const OUT_BEFORE_AFTER = fromRoot("reports", "2001_rating_before_after.csv");
const OUT_DIST = fromRoot("reports", "2001_team_distribution_before_after.csv");
const OUT_STEP6 = fromRoot("reports", "2001_distribution_warnings.csv");
...
// Authoritative 2001 team names from the team-season registry. The BEFORE csv
...
// the historically-correct 2001 name per team_code.
...
function loadAfter() {
  const files = fs
    .readdirSync(TS_DIR)
    .filter((f) => /^2001_[A-Z]{2,3}\.json$/.test(f))
    .sort();
  const teams = [];
...
// ----------------------------------------------------------------------------
function run() {
  log.step("Generating 2001 overhaul reports");

  const before = loadBefore();
  const after = loadAfter();
  const teamNames = loadTeamNames(); // 2001-correct names from the registry
  ...
  for (const team of after) {
    ...
    // 2001-correct name from the registry first; fall back to the export's own
    ...
    for (const p of players) {
      ...
      let reasonForChange = "";
      if (isOverride) {
        reasonForChange = p.ratingReason || "Manual curated override (2001 overhaul)";
      } else if (change !== "" && change !== 0) {
        reasonForChange = "Engine re-derivation after 2001 roster/override changes";
      }
      ...
      if (!isOverride && (flagged || nearStarterThin || thinStarterQB)) {
        ...
        if (QB.has(p.position)) {
          ...
          dataNeeded = "Confirm the 2001 starter (games_started) or apply a curated override; rate the starter, leave backups lower.";
...
// ----------------------------------------------------------------------------
// Step 6 realism checks (the brief's explicit warn list), scoped to 2001.
function winPct(rec) {
...
function writeSummaries(summaries) {
  const L = [];
  L.push("# 2001 NFL Season — Team Rating Summaries");
  L.push("");
  L.push("_Generated by `scripts/report-2001-overhaul.mjs` after the 2001 rating overhaul._");
  L.push("");
  L.push(
  ...
  for (const s of byTc) {
    ...
    if (flagged) {
      L.push(`_${flagged} player(s) still flagged for manual review (see \`2001_manual_review_queue.csv\`)._`);
      L.push("");
    }
  }
