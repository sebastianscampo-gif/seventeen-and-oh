// Offline gameplay smoke test for the 1999 season overhaul.
// Exercises the REAL gameplay code paths against the REAL exported 1999 JSONs:
//   1. Draft pool / catalog: index.json filtered to 1999 (what the spinner draws).
...
//   npx tsc scripts/check-1999-gameplay.ts --outDir /tmp/gp99 --module commonjs \
//     --target es2022 --moduleResolution node --skipLibCheck --esModuleInterop --resolveJsonModule
//   node /tmp/gp99/scripts/check-1999-gameplay.js
...
const pool99 = index.filter((r) => r.season === SEASON);
console.log(`Draft catalog (1999): ${pool99.length} team-seasons`);
...
console.log("\nTeam profiles (real computeTeamProfile on 1999 rosters), by power:");
...
console.log(`\nTotal 1999 players: ${allPlayers}. Most common OVR values:`);
