// Step 1 + Step 2 of the 1999 overhaul: audit the CURRENT (before) ratings and
// roster completeness for every 1999 team, grounded in the box-score snapshot
// reports/_work/1999_before.csv and the corrected registry (records/strength/
// names from data/team_seasons.csv). Read-only; writes four reports:
//
//   reports/1999_suspicious_teams.csv     distribution red-flags per team
//   reports/1999_suspicious_players.csv   players whose rating contradicts their box score
//   reports/1999_roster_completeness.csv  position-group head-count per team
//   reports/1999_rating_audit.md          narrative summary
//
// Run: node scripts/audit-1999.mjs
...
const registry = readCsv(path.join(ROOT, "data", "team_seasons.csv")).filter((r) => r.season === "1999");

const regByCode = new Map(registry.map((r) => [r.team_code, r]));
const N = (v) => (v === "" || v == null ? null : Number(v));
const ne = (v) => v != null && String(v).trim() !== "";

// Playoff fields & SB-class teams (1999) for the floor checks.
const PLAYOFF = new Set(["JAX", "IND", "SEA", "TEN", "BUF", "MIA", "LAR", "TB", "WAS", "MIN", "DET", "DAL"]);
const SB_CLASS = new Set(["LAR", "TEN", "JAX", "IND", "TB"]); // strength >= 86 / deep contenders
...
for (const code of codes) {
  ...
  const action = flags.length
    ...
    : "OK — spot-check only.";
  teamReport.push({
    team_code: code, team_name: teamName, season: 1999,
    players_total: total,
  ...
  completeness.push({
    team_code: code, team_name: teamName, season: 1999, total_players: total,
  ...
  for (const r of list) {
    ...
    if (floor && ovr != null && ovr < floor - 2) {
      playerReport.push({
        ...
        season: 1999, position: r.position, current_overall: ovr,
...
writeCsv(path.join(OUT, "1999_suspicious_teams.csv"), teamReport,
...
writeCsv(path.join(OUT, "1999_suspicious_players.csv"), playerReport,
  ["player_id", "name", "team_code", "team_name", "season", "position", "current_overall", "issue", "recommended_action"]);
writeCsv(path.join(OUT, "1999_roster_completeness.csv"), completeness,
...
const md = [];
md.push("# 1999 Rating Audit — Before State\n");
md.push(`Generated from \`reports/_work/1999_before.csv\` (${rows.length} player-rows, ${codes.length} teams) against the corrected 1999 registry.\n`);
md.push("## Summary\n");
md.push(`- **${flagged.length} of ${codes.length} teams** trip at least one distribution red-flag.`);
md.push(`- **${playerReport.length} players** carry a rating that contradicts their own 1999 box score (or are full-time starters stuck in the 67-71 band).`);
md.push(`- The dominant failure mode is **rating compression**: position groups with no recorded starts/stats collapse onto a neutral ~68 base, so genuine starters and a few stars are indistinguishable from deep backups.`);
md.push(`- **Zero curated overrides** exist for 1999 today, so every rating except the 23-man St. Louis demo roster is engine-generated.\n`);
md.push("## Position normalization status\n");
md.push("Already normalized in the source: `DE→EDGE`, interior `DL→DT`, `DB→CB`/`S`. **Not** split: offensive tackles/guards are collapsed to `LT`/`LG` (only 1 `RT` and 1 `RG` league-wide). Box scores don't distinguish L/R OL, and OL is rated as a unit, so this is cosmetic — noted, not blocking.\n");
...
md.push("\n## Recommended action\n");
md.push("Author a curated 1999 override set (stars + clear starters, ~15–25 per team) resolved to the real GSIS player_ids by name match; let true depth fall to the engine (correctly 58–70). Then regenerate, apply overrides, and re-export. The St. Louis demo roster is already a high-quality showcase but is only 23 players — extend or leave as a thin curated roster.\n");
fs.writeFileSync(path.join(OUT, "1999_rating_audit.md"), md.join("\n") + "\n");

console.log(`teams flagged: ${flagged.length}/${codes.length}`);
console.log(`suspicious players: ${playerReport.length}`);
console.log("wrote reports/1999_suspicious_teams.csv, _suspicious_players.csv, _roster_completeness.csv, _rating_audit.md");
