// report-1999-overhaul.mjs
// Read-only reporting pass for the 1999 rating overhaul. Emits, in one run:
//   Step 5: reports/1999_manual_review_queue.csv
//   Step 7: reports/1999_team_rating_summaries.md
//   Step 8: reports/1999_rating_before_after.csv
//   Step 8: reports/1999_team_distribution_before_after.csv
//
// Sources of truth:
//   BEFORE  -> reports/_work/1999_before.csv  (pre-overhaul engine state)
//   AFTER   -> public/team-seasons/1999_<TC>.json  (post-overhaul exported state)
//
// Players are joined BEFORE<->AFTER by player_id (handles the few 1999 roster
// corrections where a player moved teams). Nothing here mutates game data.
//
// Run:  node scripts/report-1999-overhaul.mjs
...
const SEASON = 1999;
const BEFORE_CSV = fromRoot("reports", "_work", "1999_before.csv");
const REGISTRY_CSV = fromRoot("data", "team_seasons.csv");
const TS_DIR = fromRoot("public", "team-seasons");

const OUT_QUEUE = fromRoot("reports", "1999_manual_review_queue.csv");
const OUT_SUMMARIES = fromRoot("reports", "1999_team_rating_summaries.md");
const OUT_BEFORE_AFTER = fromRoot("reports", "1999_rating_before_after.csv");
const OUT_DIST = fromRoot("reports", "1999_team_distribution_before_after.csv");
const OUT_STEP6 = fromRoot("reports", "1999_distribution_warnings.csv");
...
// Authoritative 1999 team names from the team-season registry. The BEFORE csv
...
// the historically-correct 1999 name per team_code.
...
function loadAfter() {
  const files = fs
    .readdirSync(TS_DIR)
    .filter((f) => /^1999_[A-Z]{2,3}\.json$/.test(f))
    .sort();
  const teams = [];
...
// ----------------------------------------------------------------------------
function run() {
  log.step("Generating 1999 overhaul reports");

  const before = loadBefore();
  const after = loadAfter();
  const teamNames = loadTeamNames(); // 1999-correct names from the registry
  ...
  for (const team of after) {
    ...
    // 1999-correct name from the registry first; fall back to the export's own
    ...
    for (const p of players) {
      ...
      let reasonForChange = "";
      if (isOverride) {
        reasonForChange = p.ratingReason || "Manual curated override (1999 overhaul)";
      } else if (change !== "" && change !== 0) {
        reasonForChange = "Engine re-derivation after 1999 roster/override changes";
      }
      ...
      if (!isOverride && (flagged || nearStarterThin || thinStarterQB)) {
        ...
        if (QB.has(p.position)) {
          ...
          dataNeeded = "Confirm the 1999 starter (games_started) or apply a curated override; rate the starter, leave backups lower.";
...
// ----------------------------------------------------------------------------
// Step 6 realism checks (the brief's explicit warn list), scoped to 1999.
function winPct(rec) {
...
function writeSummaries(summaries) {
  const L = [];
  L.push("# 1999 NFL Season — Team Rating Summaries");
  L.push("");
  L.push("_Generated by `scripts/report-1999-overhaul.mjs` after the 1999 rating overhaul._");
  L.push("");
  L.push(
  ...
  for (const s of byTc) {
    ...
    if (flagged) {
      L.push(`_${flagged} player(s) still flagged for manual review (see \`1999_manual_review_queue.csv\`)._`);
      L.push("");
    }
  }
